...
Large model checkpoints are not stored in the repository. After installing the package, see {doc}`model_checkpoints` for model download and path configuration.
...
To install the exact version used in a manuscript or release, pin the version number:

```bash
python -m pip install "cellexlink==0.1.0"
```

To upgrade an existing installation:

```bash
python -m pip install --upgrade cellexlink
```
...
## 4. Check the Python installation
...
## 5. Check the command-line interface
...
- See {doc}`input_output` for plain-text, JSON Lines, and BioC XML formats.
- See {doc}`troubleshooting` if installation or model loading fails.
