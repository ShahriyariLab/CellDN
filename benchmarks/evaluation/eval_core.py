"""Shared CellExLink benchmark evaluator.

This module contains the reusable evaluation logic shared by the benchmark
entrypoints for Cell Ontology normalization and strict end-to-end linking.
It intentionally mirrors the original CellExLink evaluation behavior. Keep
this file simple and stable: benchmark code should measure the software, not
redefine the task.
...
* The current SoftwareX evaluator exposes two reference modes:
  ``exactIDsOnly_iterator`` and ``allLabels_iterator``.
* The default evaluator mode is ``exactIDsOnly_iterator``.
...
DEFAULT_ENTITY_TYPES = ("cell_type",)
DEFAULT_ITERATOR_NAME = "exactIDsOnly_iterator"
...
def exact_ids_only_iterator(
    ...
    """Strict single-ID iterator used by default benchmark scoring."""
...
def all_labels_iterator(
    ...
    """All-label iterator that keeps every cleaned gold identifier."""
...
def evaluate_file_pair(
    *,
    ...
    iterator_name: str = DEFAULT_ITERATOR_NAME,
    topk: str = "1",
    score_threshold: Optional[float] = 0.0,
) -> list[MetricRow]:
    ...
    if iterator_name not in ITERATORS:
        raise ValueError(f"Unknown iterator: {iterator_name}")
...
def build_common_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=DEFAULT_ITERATOR_NAME,
        help="Reference iterator used for gold identifier interpretation.",
    )
