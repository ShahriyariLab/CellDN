...
python run_vaner2.py \
  --input_xml ../../data/AnatEM/test.xml \
  --model_names VANER2
```

Final XML predictions are saved under:
...
python benchmarks/evaluation/evaluate_ner.py \
  --gold AnatEM=benchmarks/data/AnatEM/test.xml \
...
for d in "${DATASETS[@]}"; do
  ARGS+=(--gold "$d=benchmarks/data/$d/test.xml")
  ARGS+=(--pred "$d=benchmarks/baselines/vaner2/model_outputs/${d}_test.xml")
done

python benchmarks/evaluation/evaluate_ner.py "${ARGS[@]}" \
  --system VANER2 \
  --macro-average \
...
- Final prediction XML files are saved in `model_outputs/`.
