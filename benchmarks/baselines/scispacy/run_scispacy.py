...
EVAL_NER = ROOT / "benchmarks" / "evaluation" / "evaluate_ner.py"
EVAL_NEN = ROOT / "benchmarks" / "evaluation" / "evaluate_nen.py"
EVAL_E2E = ROOT / "benchmarks" / "evaluation" / "evaluate_end_to_end.py"


def default_celllink_xml() -> Path:
    """Return the default CellLink XML path, supporting both common spellings."""
    candidates = [
        ROOT / "benchmarks" / "data" / "Celllink" / "test.xml",
        ROOT / "benchmarks" / "data" / "CellLink" / "test.xml",
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="CellLink BioC XML input. Default: benchmarks/data/Celllink/test.xml",
    )
    parser.add_argument(
        "--gold",
        type=Path,
